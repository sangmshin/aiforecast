...
function validateWeatherCondition(condition_new) {

    var con = condition_new.toLowerCase();
    var validation;
        ...
        deviceAddressRequest.then( (addressResponse) => {
            switch (addressResponse.statusCode) {
                ...
                case 200:
                    ...
                    try {
                        await assignCurrentCity(address['city']);
                        // response = await request('https://user-handler-service')
                    } catch (err) {
                        logger.error('AssignCurrentCity() error!', err)
                    ...
                    try {
                        ...
                        logger.error('GetAverage() error!', err)
                        return res.status(500).send()
                    } */
